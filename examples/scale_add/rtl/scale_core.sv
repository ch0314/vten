// scale_core.sv — AXI4-Stream: multiply each signed byte by scale_factor (saturating)
...
module scale_core #(parameter DATA_W = 256)(
    ...
    input  logic [7:0]  reg_scale_factor,
    ...
    // ── Data path: multiply each byte by scale_factor, signed saturation ──
    genvar i;
    generate
        for (i = 0; i < DATA_W / 8; i = i + 1) begin : scale
            wire signed [15:0] product =
                $signed(input_stream.tdata[i*8 +: 8]) * $signed({1'b0, reg_scale_factor});
            assign output_stream.tdata[i*8 +: 8] =
                (product > 16'sd127)  ? 8'sd127  :
                (product < -16'sd128) ? -8'sd128 :
                product[7:0];
        end
    endgenerate
