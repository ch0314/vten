...
class ReadDMAKernel(Kernel):
    ...
    def run(self, ctx) -> None:
        # Memory-mapped DMA: PUSH registers the DDR source buffer with the passive
        # AXI4 slave BFM and PULL arms the output-stream drain — both complete only
        # after the DUT (the AXI master) moves the bytes, which happens after start.
        # Register both buffers FIRST (no deps), then configure + start. (Gating
        # configure/pull on PUSH completion deadlocks on a real simulator; masked
        # previously by cpu-only testing.)
