...
class TestScale(TestScenario):
    ...
    def run(self, ctx, cfg):
        ...
        N = cfg.get("N", 1024)
        k = ctx.instantiate(ScaleKernel, N=N)
        k.generate_inputs(seed=42)

        total_beats = N // 32

        h_load = ctx.load_tensor(k.data_in)
        h_sf = ctx.write_register(k.ctrl, {"scale_factor": 2}, dep=h_load)
        h_len = ctx.write_register(k.ctrl, {"length": total_beats}, dep=h_sf)
        ...
        ctx.verify(h_pull, k.forward(scale_factor=2))
