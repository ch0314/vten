"""TestScenario for mm_loopback kernel.

Demonstrates AXI4 MM data flow with auto_bind address registers.
...
AXI4 MM dependency model (DUT is master, BFM is slave):
  ...
  - Configure and start must NOT depend on PUSH/PULL completion
    (they complete during DUT execution, not before).
"""
...
class TestMmLoopback(TestScenario):
    ...
    def run(self, ctx, cfg):
        ...
        from mm_loopback_kernel import MmLoopbackKernel

        N = cfg.get("N", 1024)
        k = ctx.instantiate(MmLoopbackKernel, N=N)
        ...
        #    PUSH: BFM serves data when DUT reads via AXI4
        #    PULL: BFM captures data when DUT writes via AXI4
