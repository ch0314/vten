...
class TestBrokenPassthrough(TestScenario):
    """Test broken passthrough WITHOUT probe mode.

    Host-side verify should FAIL because the RTL corrupts data
    (XORs each byte with 0x01) while forward() returns the correct
    uncorrupted output.
    """
    ...
    def run(self, ctx, cfg):
        ...
        # This verify will FAIL: RTL output != golden (forward) output
